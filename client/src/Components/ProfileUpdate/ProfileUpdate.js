  ...
  const proimage = async()=>{
    if(imgPre === ""){
      ...
      const res = await fetch('/updateprofileinitial',{
        ...
        body:JSON.stringify({
        bio, location: locationn, username: dataa
        })
      })
      navigate("/signin")
    }else{
    ...
    <div className='profileupdatemain'>
       ...
       <div className='EditProfile' onClick={()=>{navigate("/loginRegi")}}></div>
          ...
          <div className="profile_pic_center">
              ...
              {
             imgPre === ""  ?
             <img src="/profile/defaultimg.png" className="profile_pic" alt="" style={{objectFit:"cover"}}/>
             :
        ...
        <input type="file" id="pic_upload"  accept="image/png, image/jpg, image/jpeg" onChange={profilestateupload}/>
             ...
             <form className='regi_form' method='POST'>
                 ...
                 <input type="text" placeholder="Bio (optional)"  className='regi_input_profile' name="Bio" autoComplete="off" onChange={(e)=>setBio(e.target.value)}/>
                 </div>
                 <div className="regi_in">
                 <input type="text" placeholder="Location (optional"  className='regi_input_profile' name="Location" autoComplete="off" onChange={(e)=>setLocation(e.target.value)}/>
                 </div>
